...
const Login = ({ onSwitch }: { onSwitch: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    try { await login(email, password); } catch { setError("Failed to log in"); }
  };
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, backgroundColor: "white", borderTopLeftRadius: "28px", borderTopRightRadius: "28px", padding: "32px 24px", marginTop: "-28px", position: "relative", overflowY: "auto" }}>
        <h2 style={{ fontSize: "22px", fontWeight: 600, marginBottom: "6px" }}>Welcome back! 👋</h2>
        <p style={{ fontSize: "14px", color: "#667781", marginBottom: "28px" }}>Login to your account</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 16px", border: "1px solid #e0e0e0", borderRadius: "14px" }}>
              ...
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="Enter your password" style={{ flex: 1, border: "none", outline: "none", fontSize: "15px" }} />
            ...
            <div style={{ textAlign: "right", marginTop: "10px" }}><button type="button" style={{ fontSize: "13px", background: "transparent", border: "none", cursor: "pointer", color: "var(--primary-green)", fontWeight: 500 }}>Forgot password?</button></div>
          </div>
          <button type="submit" style={{ width: "100%", padding: "16px", backgroundColor: "var(--primary-green)", color: "white", border: "none", borderRadius: "14px", fontSize: "16px", fontWeight: 600, cursor: "pointer", marginTop: "8px" }}>Login</button>
        </form>
        ...
        <p style={{ textAlign: "center", fontSize: "14px", color: "#667781", marginTop: "28px" }}>Don't have an account? <button onClick={onSwitch} style={{ background: "transparent", border: "none", color: "var(--primary-green)", cursor: "pointer", fontWeight: 600 }}>Register</button></p>
      </div>
    </div>
  );
};
